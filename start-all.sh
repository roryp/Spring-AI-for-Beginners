...
if [ -z "$AZURE_OPENAI_ENDPOINT" ] || [ -z "$AZURE_OPENAI_API_KEY" ] || [ -z "$AZURE_OPENAI_DEPLOYMENT" ] || [ -z "$AZURE_OPENAI_FAST_DEPLOYMENT" ] || [ -z "$AZURE_OPENAI_EMBEDDING_DEPLOYMENT" ]; then
    echo "Error: Missing required environment variables"
    echo "Please ensure AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY, AZURE_OPENAI_DEPLOYMENT, AZURE_OPENAI_FAST_DEPLOYMENT, and AZURE_OPENAI_EMBEDDING_DEPLOYMENT are set in .env"
    exit 1
fi
